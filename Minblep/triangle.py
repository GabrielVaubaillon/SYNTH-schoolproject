...
while index < nsamples :
    ...
    if slopeIsUp :
        ...
        if phase >= b :
            # Place up to down splope change

            value = 0.5 - (phase-b) / b1 #change slope sign
            ...
            while i < (MINBLEP_PHASES * SLOPE_DD_PULSE_LENGTH) :
                if j < nsamples :
                    y[j] -= delta * (slope_dd_table[i-1] + r * (slope_dd_table[i] - slope_dd_table[i-1]))
    ...
    else : # If slope is Down

        value = 0.5 - (phase - b)/b1

        if phase >= 1.0: # Phase reset
            # Place down to up slope change
